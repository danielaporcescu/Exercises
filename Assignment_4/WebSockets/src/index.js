...
function getWarningsSince() {
  ...
  $.get(warningsSinceUrl + getFromTime(), (data) => {
    ...
    tr.setAttribute("id", "onoff");
    var warns = JSON.stringify(data.warnings);
    // let el = document.createElement("p");
    // el.setAttribute("id", "onoff");
    // el.innerText = JSON.stringify(data.warnings);
    // document.getElementById("warningSince").appendChild(el);
    ...
    ` <td>${warns.id}</td>
      ...
      document.getElementById("warningSince").appendChild(tr);
  }});
}
...
function changeSeverity() {
  ...
  if (severity === "7") {
    ...
    for (let i = 0; i < warningData.length - 1; i++) {
      if (warningData[i].prediction != null &&
